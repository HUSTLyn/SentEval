...
def batcher(params, batch):
    ...
    with torch.no_grad():
        outputs = params['model'](input_ids, attention_mask=att_mask)
        ## print('\nHeresdsadsd中文歌 飒飒大所',len(outputs),type(outputs[0]),len(outputs[0]),outputs[0].shape)
        embeddings = outputs[0][:,0]
        ## print(embeddings.shape)
        # embeddings = []
        # length = (att_mask == 1).sum(-1)
        # for i in range(outputs[0].size(0)):
        #     embeddings.append(outputs[0][i][1:length[i]-1].mean(0))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='roberta-base', help='model name or path')
